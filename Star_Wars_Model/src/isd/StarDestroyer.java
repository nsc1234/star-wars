...
public abstract class StarDestroyer  implements Maneuvers {
	...
	public void displayShipInfo() {
		...
		System.out.println("\n*********FIGHTER BAYS*********");
		int x = Collections.frequency(getShpHulls().getSectionAft().getFighterBay_PORT().getSlots(), TieFighter.TieFighterH);
		int y = Collections.frequency(getShpHulls().getSectionAft().getFighterBay_STARBOARD().getSlots(), TieFighter.TieFighterI);
		int z = Collections.frequency(getShpHulls().getSectionFwd().getFighterBay_FWD().getSlots(), TieFighter.TieFighterS);
